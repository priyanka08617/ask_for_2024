...
<input type='hidden' name='user_category_id' id='user_category_id' value='<?php echo $user_category_id;?>'>
 ...
 <div class='form-group row'>
                        ...
                        <div class='col-md-6'>
                            <select  class='form-control'   name='created_by' id='created_by' required><option value=''>Select</option>
                                <?php
                                $sql1="SELECT * FROM `users` WHERE `branch_id`='$store_id' AND `user_category_id`='3' AND `status`='1'";
...
<button type='submit'  id='submit_button' class='btn btn-primary btn-block' onclick='return confirm("Are you sure ?")'>submit</button>
   ...
   $c=1; 
              ...
              $sql='SELECT * FROM `enquiry_management` WHERE `status`="1" AND `store_id`="'.$store_id.'" AND `user_category_id`="'.$user_category_id.'" ORDER BY `id` DESC';
