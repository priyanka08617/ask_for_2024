...
$sale_or_service_check=sanitize_input($conn,$_POST['sale_or_service']);
$sale_or_service_x = $sale_or_service_check == 1 ? 'sale' : ($sale_or_service_check == 2 ? 'service' : '');
...
$store_id_x=sanitize_input($conn,$_POST['store_id']);   
$mobileNo=sanitize_input($conn,$_POST['mobileNo']);
$client_name=sanitize_input($conn,$_POST['client_name']);
$client_email=sanitize_input($conn,$_POST['client_email']);
...
$sql="INSERT INTO `customer_details`(`customer_type`, `name`, `display_name`, `mobile`, `email`, `row_created_on`,`status` ) VALUES ('1','$client_name','$client_name','$mobileNo','$client_email','$co','1')";
$query=mysqli_query($conn,$sql);
$customer_id_x=mysqli_insert_id($conn);




$sql="INSERT INTO `enquiry_management_new`(`enquiry_for`, `source_type`, `source`, `customer_id`, `enquiry_date`, `enquiry_category`, `enquiry_product`, `enquiry_detail`, `created_by`, `enquiry_status`,`store_id`,`user_category_id`, `row_created_on`, `status`) VALUES ('$sale_or_service_x','$source_type_x','$source_data_id_x','$customer_id_x', '$enquiry_date_x','$enquiry_category_x','$enquiry_product_x','$enquiry_x','$created_by_x','$enquiry_status_x','$store_id_x','$user_category_id_x','$co','1')";
