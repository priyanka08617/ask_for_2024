...
$uom_id_array=$_POST['uom_id_relation'];
...
for ($i=0; $i < count($stock_quantity) ; $i++) { 
 ...
 mysqli_query($conn,"INSERT INTO inventory_transfer_details(inventory_transfer_id, transfer_type, item_id, item_type, quantity, uom_id, status, row_created_on) VALUES ('$it_id','2','$item_id','$item_type','$stock_quantity[$i]','$uom_id_array[$i]', '2' ,'$co')");
mysqli_query($conn,"INSERT INTO stock_transfer_location(inventory_transfer_id, item_type, item_id, quantity, uom_id, transfer_type, location_id, status, row_created_on) VALUES ('$it_id','$item_type','$item_id','$stock_quantity[$i]','$uom_id_array[$i]','2','$location_id[$i]','1','$co')");
    }
}
